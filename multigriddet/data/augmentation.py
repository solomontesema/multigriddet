#!/usr/bin/python3
# -*- coding=utf-8 -*-
"""Data augmentation utility functions for MultiGridDet."""
...
def random_rotate(image, boxes, rotate_range=20, prob=0.1):
    ...
    if warpAffine and rotate_range:
        ...
        img = cv2.warpAffine(np.array(image), M, (width, height), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        ...
        [
            sometimes(iaa.OneOf([
                iaa.Affine(
                    ...
                    translate_percent={"x": (-0.10, 0.15), "y": (-0.10, 0.15)},
            ...
            iaa.SomeOf((1, 2),
                       [
                           iaa.OneOf([
                               ...
                               iaa.AdditiveGaussianNoise(loc=0, scale=(0.05 * 255, 0.15 * 255), per_channel=0.5),
                               # add gaussian noise to images
                               iaa.LinearContrast((0.7, 1.75), per_channel=0.5),
                               # improve or worsen the contrast
                               iaa.Grayscale(alpha=(0.0, 1.0))
                           ]),

                           iaa.OneOf([
                               iaa.Dropout((0.0, 0.1), per_channel=0.5),  # randomly remove up to 10% of the pixels
                               iaa.CoarseDropout((0.05, 0.1), size_percent=(0.02, 0.05), per_channel=0.5),
...
def augment_image(image, boxes, aug_pipeline):
    image = np.array(image, dtype=np.uint8)  # .reshape(bs,self.input_size,self.input_size,3)
    aug_bbs = []
    images_augmenteds = []

    seq_det = aug_pipeline
    list_bbs = []
    for box in boxes:

        x1, y1, x2, y2, c = box
        list_bbs.append(ia.BoundingBox(x1=x1, x2=x2, y1=y1, y2=y2, label=c))
    bbs = ia.BoundingBoxesOnImage(list_bbs, shape=image.shape)
    image_augmented, bbs_augmented_list = seq_det(image=image, bounding_boxes=bbs)

    fixed_bbses = [bbs_augmented_list.remove_out_of_image().clip_out_of_image()]
    boxes_augmented = [[[bb.x1, bb.y1, bb.x2, bb.y2, int(bb.label)] for bb in bbs.bounding_boxes] for bbs in fixed_bbses]
    aug_box_data = np.array(boxes_augmented).reshape((-1, 5))

    if len(boxes_augmented) == 0:
        image_augmented, aug_box_data = image, boxes  # ignore the augmentation

    return image_augmented, aug_box_data


def augmenter(images, batch_boxes):
    bs = len(images)
    images = [np.array(image, dtype=np.uint8) for image in images]  # .reshape(bs,self.input_size,self.input_size,3)
    aug_bbs = []
    images_augmenteds = []
    for image, boxes in zip(images, batch_boxes):
        seq_det = augmenter_defn_advncd()
        list_bbs = []
        for box in boxes:
            x1, y1, x2, y2, c = box
            list_bbs.append(ia.BoundingBox(x1=x1, x2=x2, y1=y1, y2=y2, label=c))
        bbs = ia.BoundingBoxesOnImage(list_bbs, shape=image.shape)
        images_augmented, bbs_augmented_list = seq_det(image=image, bounding_boxes=bbs)
        ...
        if len(boxes_augmented) == 0:
            images_augmented, aug_box_data = image, boxes  # ignore the augmentation

        aug_bbs.append(aug_box_data)
        images_augmenteds.append(images_augmented)
    return np.array(images_augmenteds), np.array(aug_bbs)


def augmenter_batch(images, batch_boxes, aug_defn):
    seq_det = aug_defn
    bs = len(images)
    ...
    for i, boxes in enumerate(batch_boxes):
        ...
        for box in boxes:
            x1, y1, x2, y2, c = box
            list_bbs.append(ia.BoundingBox(x1=x1, x2=x2, y1=y1, y2=y2, label=c))
        bbs.append(ia.BoundingBoxesOnImage(list_bbs, shape=images[i].shape))
    images_augmented, bbs_augmented_list = seq_det(images=images, bounding_boxes=bbs)

    batch_fixed_bbses = [aug_bbs_per_img.remove_out_of_image().clip_out_of_image() for aug_bbs_per_img in
                         bbs_augmented_list]
    max_num_boxes = 0
    for i, bbs in enumerate(batch_fixed_bbses):
        boxes_per_image = [[bb.x1, bb.y1, bb.x2, bb.y2, int(bb.label)] for bb in bbs.bounding_boxes]
        current = len(boxes_per_image)
        if current != 0:
            aug_bbs.append(boxes_per_image) #take the augmented boxes
        else:
            aug_bbs.append(batch_boxes[i]) #else take the original boxes
            images_augmented[i] = images[i]
        if current > max_num_boxes:
            max_num_boxes = current
    for j, boxes in enumerate(aug_bbs):
        if len(boxes)<max_num_boxes:
            for _ in range(max_num_boxes - len(boxes)):
                boxes.append([0., 0., 0., 0., 0.])
    return np.array(images_augmented), np.array(aug_bbs).reshape((bs,-1,5))
